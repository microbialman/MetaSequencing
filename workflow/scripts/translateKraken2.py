...
parser.add_argument("--taxaprefixes", dest="tpre", help="Set to 'true' if the taxonomy database used already includes p__ etc. in its taxonomy names (e.g. GTDB databases).")
args = parser.parse_args()

#check if zipped out file
compressed=False
if re.search(".gz$",args.outfile):
    args.outfile=args.outfile.strip(".gz")
    compressed=True

#get t/f from passed tpre argument
if args.tpre:
    if args.tpre in ["t","True","T","true","TRUE"]:
        args.tpre = True
    else:
        args.tpre =False

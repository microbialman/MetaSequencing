...
first=read.table(paste0(dir,"/",features[1],".tsv"),header=T,row.names=1,sep="\t")

featsum=function(feature,direct){
   cat("# ",feature,"\n")
   data=read.table(paste0(direct,"/",feature,".tsv"),header=T,row.names=1,sep="\t",comment.char = "",quote = "")
   ...
   forres=foreach(i=1:length(raredepths)) %dopar% {
     depth=raredepths[i]
